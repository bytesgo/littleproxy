...
public class ProxyToServerConnection extends ProxyConnection<HttpResponse> {
  ...
  static ProxyToServerConnection create(DefaultHttpProxyServer proxyServer, ClientToProxyConnection clientConnection,
    ...
    Queue<ProxyChain> proxyChains = new ConcurrentLinkedQueue<>();
    ProxyChainManager proxyChainManager = proxyServer.getProxyChainManager();
    if (proxyChainManager != null) {
      proxyChainManager.lookupProxyChain(initialHttpRequest, proxyChains);
      if (proxyChains.size() == 0) {
        // ChainedProxyManager returned no proxies, can't connect
        return null;
      }
    }
    return new ProxyToServerConnection(proxyServer, clientConnection, serverHostAndPort, proxyChains.poll(), proxyChains, initialFilters,
  ...
  protected ConnectionState readHTTPInitial(HttpResponse httpResponse) {
    ...
    if (httpResponse.decoderResult().isFailure()) {
      ...
      FullHttpResponse substituteResponse =
          ProxyUtil.createFullHttpResponse(HttpVersion.HTTP_1_1, HttpResponseStatus.BAD_GATEWAY, "Unable to parse response from server");
