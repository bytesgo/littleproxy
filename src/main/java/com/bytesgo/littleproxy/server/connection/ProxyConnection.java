...
public abstract class ProxyConnection<I extends HttpObject> extends SimpleChannelInboundHandler<I> {
  ...
  private void readHTTP(I httpObject) {
    ...
    switch (getCurrentState()) {
      ...
      case AWAITING_CHUNK:
        ...
        readHTTPChunk(chunk);
        nextState = ProxyUtil.isLastChunk(chunk) ? ConnectionState.AWAITING_INITIAL : ConnectionState.AWAITING_CHUNK;
        break;
      ...
      case AWAITING_CONNECT_OK:
        ...
        break;
      case HANDSHAKING:
        LOGGER.warn("Attempted to read from connection that's in the process of handshaking.  This shouldn't happen.",
            channel);
        break;
  ...
  /**
   * This method is called by users of the ProxyConnection to send stuff out over
   * the socket.
   * 
   * @param msg
   */
   ...
   * Note that for proxies with complex {@link ConnectionFlow}s that include SSL
   * handshaking and other such things, just because the {@link Channel} is
  ...
  /**
   * This method is called when the underlying {@link Channel} times out due to an
   * idle timeout.
   */
  ...
  /**
   * <p>
   * Enables tunneling on this connection by dropping the HTTP related encoders
   * and decoders, as well as idle timers.
   * </p>
   * 
   * <p>
   ...
   * </p>
   */
  protected ConnectionFlowStep<Void> StartTunneling =
  ...
  /**
   * Enables decompression and aggregation of content, which is useful for certain
   * types of filtering activity.
   * 
   * @param pipeline
   ...
   */
  protected void become(ConnectionState connectionState) {
   ...
   * 
   * By default, no-op filters are returned by DefaultHttpProxyServer. Subclasses
   * of ProxyConnection can change this behaviour.
   * 
  ...
  protected HttpFilter getHttpFiltersFromProxyServer(HttpRequest httpRequest) {
    return proxyServer.getFiltersSource()
        .filterRequest(httpRequest, ctx);
  }
  ...
  /**
   * Only once the Netty Channel is active to we recognize the ProxyConnection as
   * connected.
   */
  @Override
  ...
  /**
   * As soon as the Netty Channel is inactive, we recognize the ProxyConnection as
   * disconnected.
   */
  @Override
   ...
   * 
   * <p>
   * Note - we don't care what kind of IdleState we got. Thanks to
   * <a href="https://github.com/qbast">qbast</a> for pointing this out.
   * </p>
   */
  @Override
