...
public class ProxyToServerConnection extends ProxyConnection<HttpResponse> {
	...
	static ProxyToServerConnection create(DefaultHttpProxyServer proxyServer, ClientToProxyConnection clientConnection,
		...
		ProxyChainContext proxyChainContext = new ProxyChainContext(initialHttpRequest, clientConnection.proxyAuthorization);
		if (proxyChainManager != null) {
			proxyChainManager.lookupProxyChain(proxyChainContext);
			if (proxyChainContext.getProxyChains().size() == 0) {
				// ChainedProxyManager returned no proxies, can't connect
				return null;
			}
		}
		return new ProxyToServerConnection(proxyServer, clientConnection, serverHostAndPort, proxyChainContext.getProxyChains().poll(),
	...
	private ProxyToServerConnection(DefaultHttpProxyServer proxyServer, ClientToProxyConnection clientConnection, String serverHostAndPort,
			ProxyChain proxyChain, Queue<ProxyChain> availableChainedProxies, HttpFilter initialFilters,
			GlobalTrafficShapingHandler globalTrafficShapingHandler) throws UnknownHostException {
	...
	protected ConnectionState readHTTPInitial(HttpResponse httpResponse) {
		...
		if (httpResponse.decoderResult().isFailure()) {
			...
			FullHttpResponse substituteResponse = ProxyUtil.createFullHttpResponse(HttpVersion.HTTP_1_1, HttpResponseStatus.BAD_GATEWAY,
			...
			if (isMitmEnabled) {
				...
				if (disableSni) {
					...
					connectionFlow.then(serverConnection.EncryptChannel(
							proxyServer.getMitmManager().serverSslEngine(parsedHostAndPort.getHost(), parsedHostAndPort.getPort())));
				}
	 ...
	 */
	private ConnectionFlowStep<Void> HTTPCONNECTWithChainedProxy = new ConnectionFlowStep<Void>(this, ConnectionState.AWAITING_CONNECT_OK) {
		protected Future<Void> execute() {
	 ...
	 * </p>
	 */
	private ConnectionFlowStep<Channel> MitmEncryptClientChannel = new ConnectionFlowStep<Channel>(this, ConnectionState.HANDSHAKING) {
		@Override
		boolean shouldExecuteOnEventLoop() {
			return false;
		}
	...
	private void initChannelPipeline(ChannelPipeline pipeline, HttpRequest httpRequest) {
		...
		pipeline.addLast("decoder", new HeadAwareHttpResponseDecoder(proxyServer.getMaxInitialLineLength(), proxyServer.getMaxHeaderSize(),
	 ...
	 *                                    initial HttpRequest to the server after
	 ...
	 *                              input string could not be parsed into a host and
	 *                              port.
	 */
