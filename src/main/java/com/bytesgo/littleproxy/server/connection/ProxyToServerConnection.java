...
import com.bytesgo.littleproxy.server.connection.monitor.RequestWrittenMonitor;
import com.bytesgo.littleproxy.server.connection.monitor.ResponseReadMonitor;
...
public class ProxyToServerConnection extends ProxyConnection<HttpResponse> {
  ...
  private volatile ProxyChain proxyChain;
  private final Queue<ProxyChain> availableChainedProxies;
  ...
  /**
   * Encapsulates the flow for establishing a connection, which can vary depending
   ...
   * {@link #initializeConnectionFlow()}. This value is set to true when retrying
   * a connection without SNI to work around Java's SNI handling issue (see
   * {@link #connectionFailed(Throwable)}).
   */
  private volatile boolean disableSni = false;

  /**
   * While we're in the process of connecting, it's possible that we'll receive a
   * new message to write. This lock helps us synchronize and wait for the
   * connection to be established before writing the next message.
   */
  private final Object connectLock = new Object();

  /**
   * This is the initial request received prior to connecting. We keep track of it
   * so that we can process it after connection finishes.
   */
  private volatile HttpRequest initialRequest;

  /**
   * Keeps track of HttpRequests that have been issued so that we can associate
   * them with responses that we get back
   */
   ...
   */
  static final int MINIMUM_RECV_BUFFER_SIZE_BYTES = 64;
  ...
  static ProxyToServerConnection create(DefaultHttpProxyServer proxyServer, ClientToProxyConnection clientConnection,
    ...
    ProxyChainManager proxyChainManager = proxyServer.getProxyChainManager();
    ProxyChainContext proxyChainContext =
        new ProxyChainContext(initialHttpRequest, clientConnection.proxyAuthorization);
    if (proxyChainManager != null) {
      proxyChainManager.lookupProxyChain(proxyChainContext);
      if (proxyChainContext.getProxyChains()
          .size() == 0) {
    ...
    return new ProxyToServerConnection(proxyServer, clientConnection, serverHostAndPort,
        proxyChainContext.getProxyChains()
            .poll(),
        proxyChainContext.getProxyChains(), initialFilters, globalTrafficShapingHandler);
  }

  private ProxyToServerConnection(DefaultHttpProxyServer proxyServer, ClientToProxyConnection clientConnection,
      String serverHostAndPort, ProxyChain proxyChain, Queue<ProxyChain> availableChainedProxies,
      HttpFilter initialFilters, GlobalTrafficShapingHandler globalTrafficShapingHandler) throws UnknownHostException {
  ...
  protected ConnectionState readHTTPInitial(HttpResponse httpResponse) {
    ...
    if (httpResponse.decoderResult()
        .isFailure()) {
      LOGGER.debug("Could not parse response from server. Decoder result: {}", httpResponse.decoderResult()
          .toString());

      // create a "substitute" Bad Gateway response from the server, since we couldn't
      // understand what the actual
      // response from the server was. set the keep-alive on the substitute response
      // to false so the proxy closes
      // the connection to the server, since we don't know what state the server
      // thinks the connection is in.
      FullHttpResponse substituteResponse = ProxyUtil.createFullHttpResponse(HttpVersion.HTTP_1_1,
    ...
    if (ProxyUtil.isChunked(httpResponse)) {
  ...
  /**
   * <p>
   * Responses to HEAD requests aren't supposed to have content, but Netty doesn't
   * know that any given response is to a HEAD request, so it needs to be told
   * that there's no content so that it doesn't hang waiting for it.
   * </p>
   * 
   * <p>
   * See the documentation for {@link HttpResponseDecoder} for information about
   * why HEAD requests need special handling.
   * </p>
   * 
   * <p>
   * Thanks to <a href="https://github.com/nataliakoval">nataliakoval</a> for
   * pointing out that with connections being reused as they are, this needs to be
   * sensitive to the current request.
   * </p>
   */
  private class HeadAwareHttpResponseDecoder extends HttpResponseDecoder {
    ...
    protected boolean isContentAlwaysEmpty(HttpMessage httpMessage) {
      ...
      if (currentHttpRequest == null) {
        ...
        return ProxyUtil.isHEAD(currentHttpRequest) || super.isContentAlwaysEmpty(httpMessage);
      }
    }
  };
  ...
  /**
   * Like {@link #write(Object)} and also sets the current filters to the given
   * value.
   * 
   * @param msg
   * @param filters
  ...
  void write(Object msg) {
    ...
    if (is(ConnectionState.DISCONNECTED) && msg instanceof HttpRequest) {
      ...
      // only write this message if a connection was established and is not in the
      // process of disconnecting or
  ...
  protected void writeHttp(HttpObject httpObject) {
    if (proxyChain != null) {
      proxyChain.filterRequest(httpObject);
    }
  ...
  protected void become(ConnectionState newState) {
    ...
    } else if (getCurrentState() == ConnectionState.AWAITING_CHUNK && newState != ConnectionState.AWAITING_CHUNK) {
      currentFilters.serverToProxyResponseReceived();
    }
  ...
  protected void disconnected() {
    ...
    if (this.proxyChain != null) {
      ...
      try {
        this.proxyChain.disconnected();
      } catch (Exception e) {
        LOGGER.error("Unable to record connectionFailed", e);
      }
    }
  ...
  protected void exceptionCaught(Throwable cause) {
    try {
      if (cause instanceof IOException) {
        // IOExceptions are expected errors, for example when a server drops the
        // connection. rather than flood
        // the logs with stack traces for these expected exceptions, log the message at
        // the INFO level and the
  ...
  public InetSocketAddress getChainedProxyAddress() {
    return proxyChain == null ? null : proxyChain.getChainedProxyAddress();
  }
  ...
  /**
   * Keeps track of the current HttpResponse so that we can associate its headers
   * with future related chunks for this same transfer.
   * 
   * @param response
   */
  private void rememberCurrentResponse(HttpResponse response) {
    ...
    currentHttpResponse = ProxyUtil.copyMutableResponseFields(response);
  }
   ...
   * connection has been configured. If the {@link #disableSni} value is true,
   * this method will not pass peer information to the MitmManager when handling
   * CONNECTs.
   */
  private void initializeConnectionFlow() {
    ...
    if (ProxyUtil.isCONNECT(initialRequest)) {
      ...
      if (isMitmEnabled) {
        ...
        // SNI may be disabled for this request due to a previous failed attempt to
    ...
    protected Future<Void> execute() {
      ...
      switch (transportProtocol) {
        ...
        case UDT:
          ...
          cb.channelFactory(NettyUdtUtil.getChannel())
  ...
  private ConnectionFlowStep<Void> HTTPCONNECTWithChainedProxy =
      new ConnectionFlowStep<Void>(this, ConnectionState.AWAITING_CONNECT_OK) {
        protected Future<Void> execute() {
          ...
          boolean isMitmEnabled = mitmManager != null;
          /*
           * We ignore the LastHttpContent which we read from the client connection when
           * we are negotiating connect (see readHttp() in ProxyConnection). This cannot
           * be ignored while we are doing MITM + Chained Proxy because the
           * HttpRequestEncoder of the ProxyToServerConnection will be in an invalid state
           * when the next request is written. Writing the EmptyLastContent resets its
           * state.
           */
          if (isMitmEnabled) {
   ...
   * </p>
   */
  private ConnectionFlowStep<Channel> MitmEncryptClientChannel =
      new ConnectionFlowStep<Channel>(this, ConnectionState.HANDSHAKING) {
        @Override
        boolean shouldExecuteOnEventLoop() {
          return false;
        }
   ...
   * This method may return true to indicate that the connection should be
   * retried. If returning true, this method must set up the connection itself.
   *
  ...
  protected boolean connectionFailed(Throwable cause) throws UnknownHostException {
    // unlike a browser, java throws an exception when receiving an
    // unrecognized_name TLS warning, even if the server
    // sends back a valid certificate for the expected host. we can retry the
    // connection without SNI to allow the proxy
    // to connect to these misconfigured hosts. we should only retry the connection
    // without SNI if the connection
    // failure happened when SNI was enabled, to prevent never-ending connection
    // attempts due to SNI warnings.
    if (!disableSni && cause instanceof SSLProtocolException) {
      // unfortunately java does not expose the specific TLS alert number (112), so we
    ...
    // the connection issue wasn't due to an unrecognized_name error, or the
    // connection attempt failed even after
    // disabling SNI. before falling back to a chained proxy, re-enable SNI.
    disableSni = false;

    if (proxyChain != null) {
      LOGGER.info("Connection to upstream server via chained proxy failed", cause);
      // Let the ChainedProxy know that we were unable to connect
      proxyChain.connectionFailed(cause);
    } else {
      LOGGER.info("Connection to upstream server failed", cause);
    }

    // attempt to connect using a chained proxy, if available
    proxyChain = availableChainedProxies.poll();
    if (proxyChain != null) {
      LOGGER.info("Retrying connecting using the next available chained proxy");
  ...
  /**
   * Convenience method to prepare to retry this connection. Closes the
   * connection's channel and sets up the connection again using
   * {@link #setupConnectionParameters()}.
   *
   * @throws UnknownHostException when {@link #setupConnectionParameters()} is
   ...
   * 
   * @throws UnknownHostException when unable to resolve the hostname to an IP
   *         address
   */
  private void setupConnectionParameters() throws UnknownHostException {
    if (proxyChain != null && proxyChain != ProxyChainAdapter.FALLBACK_TO_DIRECT_CONNECTION) {
      this.transportProtocol = proxyChain.getTransportProtocol();
      this.remoteAddress = proxyChain.getChainedProxyAddress();
      this.localAddress = proxyChain.getLocalAddress();
    } else {
      ...
      // save the hostname and port of the unresolved address in hostAndPort, in case
      ...
      try {
        if (this.remoteAddress == null) {
          ...
          // filter returned an unresolved address, so resolve it using the proxy server's
          // resolver
        ...
        // unable to resolve the hostname to an IP address. notify the filters of the
        // failure before allowing the
   ...
   * {@link HttpResponseEncoder} or {@link HttpRequestEncoder} before the
  ...
  /**
   * <p>
   * Do all the stuff that needs to be done after our {@link ConnectionFlow} has
   * succeeded.
   * </p>
   * 
   * @param shouldForwardInitialRequest whether or not we should forward the
   *        initial HttpRequest to the server after the connection has been
   *        established.
   */
  void connectionSucceeded(boolean shouldForwardInitialRequest) {
    ...
    if (this.proxyChain != null) {
      ...
      try {
        this.proxyChain.connectionSucceeded();
      } catch (Exception e) {
        LOGGER.error("Unable to record connectionSucceeded", e);
      }
    }
    ...
    // we're now done with the initialRequest: it's either been forwarded to the
    // upstream server (HTTP requests), or
    // completely dropped (HTTPS CONNECTs). if the initialRequest is reference
    // counted (typically because the
   ...
   * @throws UnknownHostException if hostAndPort could not be resolved, or if the
   *         input string could not be parsed into a host and port.
   */
  public static InetSocketAddress addressFor(String hostAndPort, DefaultHttpProxyServer proxyServer)
    ...
    try {
      ...
      // we couldn't understand the hostAndPort string, so there is no way we can
      // resolve it.
    ...
    return proxyServer.getServerResolver()
        .resolve(host, port);
  }
  ...
  private ResponseReadMonitor responseReadMonitor = new ResponseReadMonitor(LOGGER) {
    @Override
