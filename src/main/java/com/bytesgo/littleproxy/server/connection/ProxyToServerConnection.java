...
import static com.bytesgo.littleproxy.model.enums.ConnectionState.AWAITING_CONNECT_OK;
import static com.bytesgo.littleproxy.model.enums.ConnectionState.AWAITING_INITIAL;
import static com.bytesgo.littleproxy.model.enums.ConnectionState.CONNECTING;
import static com.bytesgo.littleproxy.model.enums.ConnectionState.DISCONNECTED;
import static com.bytesgo.littleproxy.model.enums.ConnectionState.HANDSHAKING;
...
public class ProxyToServerConnection extends ProxyConnection<HttpResponse> {
    ...
    static ProxyToServerConnection create(DefaultHttpProxyServer proxyServer, ClientToProxyConnection clientConnection,
        ...
        ProxyChainContext proxyChainContext = new ProxyChainContext(initialHttpRequest,
                clientConnection.proxyAuthorization);
        ...
        return new ProxyToServerConnection(proxyServer, clientConnection, serverHostAndPort,
                proxyChainContext.getProxyChains().poll(), proxyChainContext.getProxyChains(), initialFilters,
    ...
    private ProxyToServerConnection(DefaultHttpProxyServer proxyServer, ClientToProxyConnection clientConnection,
            ...
            HttpFilter initialFilters, GlobalTrafficShapingHandler globalTrafficShapingHandler)
            throws UnknownHostException {
    ...
    private void initializeConnectionFlow() {
        ...
        if (ProxyUtil.isCONNECT(initialRequest)) {
            ...
            if (isMitmEnabled) {
                ...
                if (disableSni) {
                    connectionFlow
                            .then(serverConnection.EncryptChannel(proxyServer.getMitmManager().serverSslEngine()));
                } else {
     ...
     */
    private ConnectionFlowStep<Void> HTTPCONNECTWithChainedProxy = new ConnectionFlowStep<Void>(this,
            AWAITING_CONNECT_OK) {
        protected Future<Void> execute() {
    ...
    private ConnectionFlowStep<Channel> MitmEncryptClientChannel = new ConnectionFlowStep<Channel>(this, HANDSHAKING) {
        ...
        protected Future<Channel> execute() {
            return clientConnection
                    .encrypt(proxyServer.getMitmManager().clientSslEngineFor(initialRequest, sslEngine.getSession()),
                            false)
