...
public class ClientToProxyConnection extends ProxyConnection<HttpRequest> {
	 ...
	 */
	private static final String LOWERCASE_TRANSFER_ENCODING_HEADER = HttpHeaderNames.TRANSFER_ENCODING.toLowerCase().toString();
	...
	public ClientToProxyConnection(final DefaultHttpProxyServer proxyServer, SslEngineSource sslEngineSource, boolean authenticateClients,
	...
	private ConnectionState doReadHTTPInitial(HttpRequest httpRequest) {
		...
		if (newConnectionRequired) {
			try {
				currentServerConnection = ProxyToServerConnection.create(proxyServer, this, serverHostAndPort, currentFilters, httpRequest,
	 ...
	 *                            doing chunked transfers, this is the initial
	 *                            HttpResponse object that came in before the other
	 *                            chunks)
	 ...
	 */
	ConnectionFlowStep<Void> RespondCONNECTSuccessful = new ConnectionFlowStep<Void>(this, ConnectionState.NEGOTIATING_CONNECT) {
		@Override
	...
	private void initChannelPipeline(ChannelPipeline pipeline) {
		...
		// respectively.
		pipeline.addLast("decoder", new HttpRequestDecoder(proxyServer.getMaxInitialLineLength(), proxyServer.getMaxHeaderSize(),
	...
	private void writeAuthenticationRequired(String realm) {
		...
		FullHttpResponse response = ProxyUtil.createFullHttpResponse(HttpVersion.HTTP_1_1, HttpResponseStatus.PROXY_AUTHENTICATION_REQUIRED,
				body);
		response.headers().set(HttpHeaderNames.DATE, new Date());
		response.headers().set("Proxy-Authenticate", "Basic realm=\"" + (realm == null ? "Restricted Files" : realm) + "\"");
		write(response);
	}
...
//			return ((FullHttpRequest) original).copy();
	...
	private boolean writeBadGateway(HttpRequest httpRequest) {
		...
		FullHttpResponse response = ProxyUtil.createFullHttpResponse(HttpVersion.HTTP_1_1, HttpResponseStatus.BAD_GATEWAY, body);
	...
	private boolean writeBadRequest(HttpRequest httpRequest) {
		...
		FullHttpResponse response = ProxyUtil.createFullHttpResponse(HttpVersion.HTTP_1_1, HttpResponseStatus.BAD_REQUEST, body);
	...
	private boolean writeGatewayTimeout(HttpRequest httpRequest) {
		...
		FullHttpResponse response = ProxyUtil.createFullHttpResponse(HttpVersion.HTTP_1_1, HttpResponseStatus.GATEWAY_TIMEOUT, body);
