...
import com.bytesgo.littleproxy.server.connection.monitor.BytesReadMonitor;
import com.bytesgo.littleproxy.server.connection.monitor.BytesWrittenMonitor;
import com.bytesgo.littleproxy.server.connection.monitor.RequestReadMonitor;
import com.bytesgo.littleproxy.server.connection.monitor.ResponseWrittenMonitor;
...
public class ClientToProxyConnection extends AbstractProxyConnection<HttpRequest> implements ConnectionLifecycle<HttpRequest> {
   ...
   */
  private static final String LOWERCASE_TRANSFER_ENCODING_HEADER = HttpHeaderNames.TRANSFER_ENCODING.toLowerCase()
      .toString();
  ...
  protected ProxyAuthorization proxyAuthorization = new ProxyAuthorization();
  ...
  public ClientToProxyConnection(final DefaultHttpProxyServer proxyServer, SslEngineSource sslEngineSource,
      boolean authenticateClients, ChannelPipeline pipeline, GlobalTrafficShapingHandler globalTrafficShapingHandler) {
  ...
  public ConnectionState readHTTPInitial(HttpRequest httpRequest) {
    ...
    // connection, since we do not know what state
    // the client thinks the connection is in
    if (httpRequest.decoderResult()
        .isFailure()) {
      LOGGER.debug("Could not parse request from client. Decoder result: {}", httpRequest.decoderResult()
          .toString());

      FullHttpResponse response = ProxyUtil.createFullHttpResponse(HttpVersion.HTTP_1_1, HttpResponseStatus.BAD_REQUEST,
   ...
   * 
   * <p>
   * Note - the "server" could be a chained proxy, not the final endpoint for the
   * request.
   * </p>
   * 
   * @param httpRequest
  ...
  private ConnectionState doReadHTTPInitial(HttpRequest httpRequest) {
    ...
    // Set up our filters based on the original request. If the HttpFiltersSource
    // returns null (meaning the request/response
    // should not be filtered), fall back to the default no-op filter source.
    HttpFilter filterInstance = proxyServer.getFiltersSource()
        .filterRequest(currentRequest, ctx);
    ...
    // Send the request through the clientToProxyRequest filter, and respond with
    // the short-circuit response if required
    ...
    // if origin-form requests are not explicitly enabled, short-circuit requests
    // that treat the proxy as the
    ...
    LOGGER.debug("Ensuring that hostAndPort are available in {}", httpRequest.uri());
    ...
    if (ProxyUtil.isCONNECT(httpRequest)) {
      LOGGER.debug("Not reusing existing ProxyToServerConnection because request is a CONNECT for: {}",
          serverHostAndPort);
      newConnectionRequired = true;
    } else if (currentServerConnection == null) {
      LOGGER.debug("Didn't find existing ProxyToServerConnection for: {}", serverHostAndPort);
      newConnectionRequired = true;
    }

    if (newConnectionRequired) {
      try {
        currentServerConnection = ProxyToServerConnection.create(proxyServer, this, serverHostAndPort, currentFilters,
            httpRequest, globalTrafficShapingHandler);
        ...
        LOGGER.info("Bad Host {}", httpRequest.uri());
        boolean keepAlive = writeBadGateway(httpRequest);
        resumeReading();
        if (keepAlive) {
          return ConnectionState.AWAITING_INITIAL;
    ...
    } else if (ProxyUtil.isChunked(httpRequest)) {
  ...
  /**
   * Returns true if the specified request is a request to an origin server,
   * rather than to a proxy server. If this request is being MITM'd, this method
   * always returns false. The format of requests to a proxy server are defined in
   * RFC 7230, section 5.3.2 (all other requests are considered requests to an
   * origin server):
   * 
   * <pre>
   ...
   * @return true if the specified request is a request to an origin server,
   *         otherwise false
   */
  private boolean isRequestToOriginServer(HttpRequest httpRequest) {
    // while MITMing, all HTTPS requests are requests to the origin server, since
    ...
    return !HTTP_SCHEME.matcher(uri)
        .matches();
  }
   ...
   * @param currentHttpResponse the HttpResponse corresponding to this data (when
   *        doing chunked transfers, this is the initial HttpResponse object that
   *        came in before the other chunks)
  ...
  void respond(ProxyToServerConnection serverConnection, HttpFilter filters, HttpRequest currentHttpRequest,
    ...
    if (httpObject instanceof HttpResponse) {
      ...
      // if this HttpResponse does not have any means of signaling the end of the
      // message body other than closing
      // the connection, convert the message to a "Transfer-Encoding: chunked" HTTP
      // response. This avoids the need
      // to close the client connection to indicate the end of the message. (Responses
      // to HEAD requests "must be" empty.)
      if (!ProxyUtil.isHEAD(currentHttpRequest) && !ProxyUtil.isResponseSelfTerminating(httpResponse)) {
        // if this is not a FullHttpResponse, duplicate the HttpResponse from the server
        // before sending it to
        // the client. this allows us to set the Transfer-Encoding to chunked without
        // interfering with netty's
        // handling of the response from the server. if we modify the original
        // HttpResponse from the server,
        // netty will not generate the appropriate LastHttpContent when it detects the
        // connection closure from
        // the server (see HttpObjectDecoder#decodeLast). (This does not apply to
        // FullHttpResponses, for which
        // netty already generates the empty final chunk when Transfer-Encoding is
        // chunked.)
        if (!(httpResponse instanceof FullHttpResponse)) {
          HttpResponse duplicateResponse = ProxyUtil.duplicateHttpResponse(httpResponse);

          // set the httpObject and httpResponse to the duplicated response, to allow all
    ...
    if (ProxyUtil.isLastChunk(httpObject)) {
      writeEmptyBuffer();
    }
   ...
   */
  ConnectionFlowStep<Void> RespondCONNECTSuccessful =
      new ConnectionFlowStep<Void>(this, ConnectionState.NEGOTIATING_CONNECT) {
        @Override
        ...
        protected ChannelFuture execute() {
          ...
          HttpResponse response = ProxyUtil.createFullHttpResponse(HttpVersion.HTTP_1_1, CONNECTION_ESTABLISHED);
          response.headers()
              .set(HttpHeaderNames.CONNECTION, HttpHeaderValues.KEEP_ALIVE);
          ProxyUtil.addVia(response, proxyServer.getProxyAlias());
  ...
  void timedOut(ProxyToServerConnection serverConnection) {
    if (currentServerConnection == serverConnection && this.lastReadTime > currentServerConnection.lastReadTime) {
      // the idle timeout fired on the active server connection. send a timeout
  ...
  protected void timedOut() {
    // idle timeout fired on the client channel. if we aren't waiting on a response
    // from a server, hang up
  ...
  protected void serverConnectionSucceeded(ProxyToServerConnection serverConnection,
    ...
    become(shouldForwardInitialRequest ? getCurrentState() : ConnectionState.AWAITING_INITIAL);
   ...
   * <li>If the server was the ultimate endpoint, we return a 502 Bad Gateway to
   * the client.</li>
   * </ol>
   * 
   * @param serverConnection
  ...
  protected boolean serverConnectionFailed(ProxyToServerConnection serverConnection,
    ...
    try {
      ...
      if (retrying) {
        ...
        LOGGER.debug("Connection to upstream server or chained proxy failed: {}.  Last state before failure: {}",
  ...
  /**
   * On disconnect of the server, track that we have one fewer connected servers
   * and then disconnect the client if necessary.
   * 
   * @param serverConnection
   */
  protected void serverDisconnected(ProxyToServerConnection serverConnection) {
    ...
    // for non-SSL connections, do not disconnect the client from the proxy, even if
    // this was the last server connection.
    // this allows clients to continue to use the open connection to the proxy to
    // make future requests. for SSL
    // connections, whether we are tunneling or MITMing, we need to disconnect the
    // client because there is always
    // exactly one ClientToProxyConnection per ProxyToServerConnection, and vice
    // versa.
  ...
  /**
   * When a server becomes writeable, we check to see if all servers are writeable
   * and if they are, we resume reading.
   * 
   * @param serverConnection
   */
  ...
  protected void exceptionCaught(Throwable cause) {
    try {
      if (cause instanceof IOException) {
        // IOExceptions are expected errors, for example when a browser is killed and
        // aborts a connection.
        // rather than flood the logs with stack traces for these expected exceptions,
        // we log the message at the
   ...
   * 
   * A {@link ChannelPipeline} invokes the read (Inbound) handlers in ascending
   * ordering of the list and then the write (Outbound) handlers in descending
   * ordering.
   * 
   * Regarding the Javadoc of {@link HttpObjectAggregator} it's needed to have the
   * {@link HttpResponseEncoder} or
   * {@link io.netty.handler.codec.http.HttpRequestEncoder} before the
  ...
  private boolean shouldCloseClientConnection(HttpRequest req, HttpResponse res, HttpObject httpObject) {
    ...
    // ignore the response's keep-alive; we can keep this client connection open as
    // long as the client allows it.
  ...
  /**
   * Determines if the remote connection should be closed based on the request and
   * response pair. If the request is HTTP 1.0 with no keep-alive header, for
   * example, the connection should be closed.
   * 
  ...
  private boolean shouldCloseServerConnection(HttpRequest req, HttpResponse res, HttpObject msg) {
    ...
    // ignore the request's keep-alive; we can keep this server connection open as
    // long as the server allows it.
  ...
  private boolean authenticationRequired(HttpRequest request) {
    ...
    if (!request.headers()
        .contains(HttpHeaderNames.PROXY_AUTHORIZATION)) {
      writeAuthenticationRequired(authenticator.getRealm());
      return true;
    }

    List<String> values = request.headers()
        .getAll(HttpHeaderNames.PROXY_AUTHORIZATION);
    String fullValue = values.iterator()
        .next();
    String value = StringUtils.substringAfter(fullValue, "Basic ")
        .trim();

    byte[] decodedValue = BaseEncoding.base64()
        .decode(value);
    ...
    proxyAuthorization.setUsername(userName);
    ...
    String authentication = request.headers()
        .get(HttpHeaderNames.PROXY_AUTHORIZATION);
  ...
  private void writeAuthenticationRequired(String realm) {
    ...
    FullHttpResponse response =
        ProxyUtil.createFullHttpResponse(HttpVersion.HTTP_1_1, HttpResponseStatus.PROXY_AUTHENTICATION_REQUIRED, body);
    response.headers()
        .set(HttpHeaderNames.DATE, new Date());
    response.headers()
        .set("Proxy-Authenticate", "Basic realm=\"" + (realm == null ? "Restricted Files" : realm) + "\"");
    write(response);
  }
   ...
   * response that reports its HTTP version as 1.0. In this case, we change it to
   * 1.1.
   * 
   * @param httpResponse
   */
  private void fixHttpVersionHeaderIfNecessary(HttpResponse httpResponse) {
    String te = httpResponse.headers()
        .get(HttpHeaderNames.TRANSFER_ENCODING);
  ...
  private void modifyRequestHeadersToReflectProxying(HttpRequest httpRequest) {
    if (!currentServerConnection.hasUpstreamChainedProxy()) {
      /*
       * We are making the request to the origin server, so must modify the
       * 'absolute-URI' into the 'origin-form' as per RFC 7230 section 5.3.1.
       *
       * This must happen even for 'transparent' mode, otherwise the origin server
       * could infer that the request came via a proxy server.
       */
      LOGGER.debug("Modifying request for proxy chaining");
      // Strip host from uri
      String uri = httpRequest.uri();
      String adjustedUri = ProxyUtil.stripHost(uri);
    ...
    if (!proxyServer.isTransparent()) {
      ...
      ProxyUtil.removeSdchEncoding(headers);
      switchProxyConnectionHeader(headers);
      stripConnectionTokens(headers);
      stripHopByHopHeaders(headers);
      ProxyUtil.addVia(httpRequest, proxyServer.getProxyAlias());
    }
  }
  ...
  private void modifyResponseHeadersToReflectProxying(HttpResponse httpResponse) {
    if (!proxyServer.isTransparent()) {
      ...
      stripConnectionTokens(headers);
      stripHopByHopHeaders(headers);
      ProxyUtil.addVia(httpResponse, proxyServer.getProxyAlias());

      /*
       * RFC2616 Section 14.18
       * 
       * A received message that does not have a Date header field MUST be assigned
       * one by the recipient if the message will be cached by that recipient or
  ...
  /**
   * Switch the de-facto standard "Proxy-Connection" header to "Connection" when
   * we pass it along to the remote host. This is largely undocumented but seems
   * to be what most browsers and servers expect.
   * 
  ...
  private void stripConnectionTokens(HttpHeaders headers) {
    if (headers.contains(HttpHeaderNames.CONNECTION)) {
      for (String headerValue : headers.getAll(HttpHeaderNames.CONNECTION)) {
        for (String connectionToken : ProxyUtil.splitCommaSeparatedHeaderValues(headerValue)) {
          // do not strip out the Transfer-Encoding header if it is specified in the
          // Connection header, since LittleProxy does not
  ...
  private void stripHopByHopHeaders(HttpHeaders headers) {
    ...
    for (String headerName : headerNames) {
      if (ProxyUtil.shouldRemoveHopByHopHeader(headerName)) {
  ...
  /**
   * Tells the client that something went wrong trying to proxy its request. If
   * the Bad Gateway is a response to an HTTP HEAD request, the response will
   * contain no body, but the Content-Length header will be set to the value it
   * would have been if this 502 Bad Gateway were in response to a GET.
   *
   * @param httpRequest the HttpRequest that is resulting in the Bad Gateway
   *        response
   * @return true if the connection will be kept open, or false if it will be
   *         disconnected
   */
  private boolean writeBadGateway(HttpRequest httpRequest) {
    String body = "Bad Gateway: " + httpRequest.uri();
    FullHttpResponse response =
        ProxyUtil.createFullHttpResponse(HttpVersion.HTTP_1_1, HttpResponseStatus.BAD_GATEWAY, body);

    if (ProxyUtil.isHEAD(httpRequest)) {
  ...
  /**
   * Tells the client that the request was malformed or erroneous. If the Bad
   * Request is a response to an HTTP HEAD request, the response will contain no
   * body, but the Content-Length header will be set to the value it would have
  ...
  private boolean writeBadRequest(HttpRequest httpRequest) {
    ...
    FullHttpResponse response =
        ProxyUtil.createFullHttpResponse(HttpVersion.HTTP_1_1, HttpResponseStatus.BAD_REQUEST, body);
  ...
  /**
   * Tells the client that the connection to the server, or possibly to some
   * intermediary service (such as DNS), timed out. If the Gateway Timeout is a
   * response to an HTTP HEAD request, the response will contain no body, but the
   * Content-Length header will be set to the value it would have been if this 504
   * Gateway Timeout were in response to a GET.
   *
   * @param httpRequest the HttpRequest that is resulting in the Gateway Timeout
   *        response
   * @return true if the connection will be kept open, or false if it will be
   *         disconnected
   */
  private boolean writeGatewayTimeout(HttpRequest httpRequest) {
    ...
    FullHttpResponse response =
        ProxyUtil.createFullHttpResponse(HttpVersion.HTTP_1_1, HttpResponseStatus.GATEWAY_TIMEOUT, body);

    if (httpRequest != null && ProxyUtil.isHEAD(httpRequest)) {
      // don't allow any body content in response to a HEAD request
      response.content()
          .clear();
    }
  ...
  /**
   * Responds to the client with the specified "short-circuit" response. The
   * response will be sent through the
   * {@link HttpFilter#proxyToClientResponse(HttpObject)} filter method before
   * writing it to the client. The client will not be disconnected, unless the
   * response includes a "Connection: close" header, or the filter returns a null
   * HttpResponse (in which case no response will be written to the client and the
   * connection will be disconnected immediately). If the response is not a Bad
   * Gateway or Gateway Timeout response, the response's headers will be modified
   * to reflect proxying, including adding a Via header, Date header, etc.
   *
  ...
  private boolean respondWithShortCircuitResponse(HttpResponse httpResponse) {
    ...
    // allow short-circuit messages to close the connection. normally the Connection
    // header would be stripped when modifying
    // the message for proxying, so save the keep-alive status before the
    // modifications are made.
    boolean isKeepAlive = HttpUtil.isKeepAlive(httpResponse);

    // if the response is not a Bad Gateway or Gateway Timeout, modify the headers
    // "as if" the short-circuit response were
    // proxied
    ...
    // restore the keep alive status, if it was overwritten when modifying headers
    // for proxying
    HttpUtil.setKeepAlive(httpResponse, isKeepAlive);

    write(httpResponse);

    if (ProxyUtil.isLastChunk(httpResponse)) {
      writeEmptyBuffer();
    }
  ...
  private String identifyHostAndPort(HttpRequest httpRequest) {
    String hostAndPort = ProxyUtil.parseHostAndPort(httpRequest);
    if (StringUtils.isBlank(hostAndPort)) {
      List<String> hosts = httpRequest.headers()
          .getAll(HttpHeaderNames.HOST);
  ...
  /**
   * Write an empty buffer at the end of a chunked transfer. We need to do this to
   * handle the way Netty creates HttpChunks from responses that aren't in fact
   * chunked from the remote server using Transfer-Encoding: chunked. Netty turns
   * these into pseudo-chunked responses in cases where the response would
   * otherwise fill up too much memory or where the length of the response body is
   * unknown. This is handy because it means we can start streaming response
   * bodies back to the client without reading the entire response. The problem is
   * that in these pseudo-cases the last chunk is encoded to null, and this
   * thwarts normal ChannelFutures from propagating operationComplete events on
   * writes to appropriate channel listeners. We work around this by writing an
   * empty buffer in those cases and using the empty buffer's future instead to
   * handle any operations we need to when responses are fully written back to
   * clients.
   */
   ...
   * 
   * We track statistics on bytes, requests and responses by adding handlers at
   * the appropriate parts of the pipeline (see initChannelPipeline()).
   **************************************************************************/
  private final BytesReadMonitor bytesReadMonitor = new BytesReadMonitor(LOGGER) {
    @Override
  ...
  private RequestReadMonitor requestReadMonitor = new RequestReadMonitor(LOGGER) {
    @Override
  ...
  private BytesWrittenMonitor bytesWrittenMonitor = new BytesWrittenMonitor(LOGGER) {
    @Override
  ...
  private ResponseWrittenMonitor responseWrittenMonitor = new ResponseWrittenMonitor(LOGGER) {
    @Override
