 ...
 * {@link DefaultHttpProxyServer#start()}. For example:
 * </p>
 *
 * <pre>
 * DefaultHttpProxyServer server = DefaultHttpProxyServer.bootstrap()
 *     .withPort(8090)
 *     .start();
 * </pre>
 *
...
public class DefaultHttpProxyServer implements HttpProxyServer {
   ...
   * compute and throttle the proxy-to-server bandwidth.
   */
   ...
   * specified and the hostname of the local machine cannot be resolved.
   */
   ...
   * requestedAddress in some circumstances, for example when the requested port
   * is 0.
   */
  ...
  private final ProxyChainManager chainProxyManager;
   ...
   * variable to allow removing the shutdown hook when the proxy server is stopped
   * normally.
   */
   ...
   * @param sslEngineSource (optional) if specified, this Proxy will encrypt
   *        inbound connections from clients using an {@link SSLEngine} obtained
   *        from this {@link SslEngineSource}.
   * @param authenticateSslClients Indicate whether or not to authenticate clients
   *        when using SSL
   * @param proxyAuthenticator (optional) If specified, requests to the proxy will
   *        be authenticated using HTTP BASIC authentication per the provided
   *        {@link ProxyAuthenticator}
   * @param chainProxyManager The proxy to send requests to if chaining proxies.
   *        Typically <code>null</code>.
   * @param mitmManager The {@link MitmManager} to use for man in the middle'ing
   *        CONNECT requests
   * @param filtersSource Source for {@link HttpFilter}
   * @param transparent If true, this proxy will run as a transparent proxy. This
   *        will not modify the response, and will only modify the request to
   *        amend the URI if the target is the origin server (to comply with RFC
   *        7230 section 5.3.1).
   * @param idleConnectionTimeout The timeout (in seconds) for auto-closing idle
   *        connections.
   * @param activityTrackers for tracking activity on this proxy
   * @param connectTimeout number of milliseconds to wait to connect to the
   *        upstream server
   * @param serverResolver the {@link HostResolver} to use for resolving server
   *        addresses
   ...
   *        requests that contain an origin-form URI, as defined in RFC 7230 5.3.1
   */
  public DefaultHttpProxyServer(ServerGroup serverGroup, TransportProtocol transportProtocol,
      InetSocketAddress requestedAddress, SslEngineSource sslEngineSource, boolean authenticateSslClients,
      ProxyAuthenticator proxyAuthenticator, ProxyChainManager chainProxyManager, MitmManager mitmManager,
      ...
      long readThrottleBytesPerSecond, long writeThrottleBytesPerSecond, InetSocketAddress localAddress,
      String proxyAlias, int maxInitialLineLength, int maxHeaderSize, int maxChunkSize,
    ...
    if (proxyAlias == null) {
      ...
      String hostname = ProxyUtil.getHostName();
      if (hostname == null) {
  ...
  public void setThrottle(long readThrottleBytesPerSecond, long writeThrottleBytesPerSecond) {
    if (globalTrafficShapingHandler != null) {
      ...
      if (readThrottleBytesPerSecond > 0 || writeThrottleBytesPerSecond > 0) {
        globalTrafficShapingHandler = createGlobalTrafficShapingHandler(transportProtocol, readThrottleBytesPerSecond,
  ...
  public HttpProxyServerBootstrap clone() {
    return new DefaultHttpProxyServerBootstrap(serverGroup, transportProtocol,
        ...
        sslEngineSource, authenticateSslClients, proxyAuthenticator, chainProxyManager, mitmManager, filtersSource,
   ...
   * the server and unregisters this server from the server group.
   *
  ...
  protected void doStop(boolean graceful) {
    ...
    if (stopped.compareAndSet(false, true)) {
      ...
      try {
        Runtime.getRuntime()
            .removeShutdownHook(jvmShutdownHook);
  ...
  private void doStart() {
    EventLoopGroup boosGroup = serverGroup.getClientToProxyAcceptor(transportProtocol);
    EventLoopGroup workerGroup = serverGroup.getClientToProxyWorker(transportProtocol);

    ServerBootstrap serverBootstrap = new ServerBootstrap().group(boosGroup, workerGroup);
    ...
    switch (transportProtocol) {
      ...
      case UDT:
        ...
        serverBootstrap.channelFactory(NettyUdtUtil.getServerChannel())
    ...
    this.boundAddress = ((InetSocketAddress) future.channel()
        .localAddress());
    LOG.info("Proxy started at address: " + this.boundAddress);

    Runtime.getRuntime()
        .addShutdownHook(jvmShutdownHook);
  }

  public ProxyChainManager getProxyChainManager() {
  ...
  public EventLoopGroup getProxyToServerWorkerFor(TransportProtocol transportProtocol) {
    return serverGroup.getProxyToServerWorker(transportProtocol);
  }
