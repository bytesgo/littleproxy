...
public class ServerGroup {
   ...
   */
  public final List<HttpProxyServer> httpProxyServers = new ArrayList<HttpProxyServer>(1);
  ...
  private static final EnumMap<TransportProtocol, SelectorProvider> TRANSPORT_PROTOCOL_SELECTOR_PROVIDERS =
      new EnumMap<TransportProtocol, SelectorProvider>(TransportProtocol.class);
  static {
   ...
   * @param proxyServerConfiguration proxyServerConfiguration
   */
    ...
    if (protocolThreadPools.get(protocol) == null) {
      synchronized (THREAD_POOL_INIT_LOCK) {
        if (protocolThreadPools.get(protocol) == null) {
          ...
          ProxyThreadPool threadPool = new ProxyThreadPool(selectorProvider, incomingAcceptorThreadSize,
              incomingWorkerThreadSize, outgoingWorkerThreadSize, name, serverGroupId);
   ...
   * @param graceful when true, the server group shutdown (if necessary) will be
   *        graceful
   */
