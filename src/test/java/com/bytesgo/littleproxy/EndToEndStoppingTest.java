...
/**
 * End to end test making sure the proxy is able to service simple HTTP requests and stop at the end. Made into a unit
 * test from isopov and nasis's contributions at: https://github.com/adamfisk/LittleProxy/issues/36
 */
public class EndToEndStoppingTest {
  ...
  /**
   * This is a quick test from nasis that exhibits different behavior from unit tests because unit tests call
   * System.exit(). The stop method should stop all non-daemon threads and should cause the JVM to exit without explicitly
   * calling System.exit(), which running as an application properly tests.
   */
  ...
  private void runSiteTestWithHttpClient(final String site) throws Exception {
    ...
    /*
     * final HttpProxyServer ssl = new DefaultHttpProxyServer(PROXY_PORT, null, null, new SslHandshakeHandlerFactory(), new
     * HttpRequestFilter() {
     * 
     * @Override public void filter(HttpRequest httpRequest) { System.out.println("Request went through proxy"); } });
     */

    final HttpProxyServer proxy = DefaultHttpProxyServer.bootstrap().withPort(0).withFiltersSource(new HttpFilterSourceAdapter() {
      @Override
    ...
    try {
      ...
      final String body = IOUtils.toString(entity.getContent(), Charset.defaultCharset());
  ...
  public void testWithWebDriver() throws Exception {
    ...
    MatcherAssert.assertThat(source.length(), greaterThan(100));
    driver.close();
