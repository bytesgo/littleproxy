...
public class ThrottlingTest {
  ...
  public void aWarmUpTest() throws IOException {
    ...
    CloseableHttpClient httpClient = createHttpClient(proxyPort);
  ...
  public void testThrottledWrite() throws Exception {
    ...
    final org.apache.http.HttpResponse response = httpClient.execute(new HttpHost("127.0.0.1", writeWebServerPort), request);
    long finish = System.currentTimeMillis();

    assertEquals("Received " + largeData.length + " bytes\n", EntityUtils.toString(response.getEntity()));

    MatcherAssert.assertThat(
        "Expected throttled write to complete in approximately " + msToWriteThrottled + "ms" + " but took " + (finish - start) + "ms",
        (double) (finish - start),
        both(greaterThan(msToWriteThrottled * (1 - ALLOWABLE_VARIATION))).and(lessThan(msToWriteThrottled * (1 + ALLOWABLE_VARIATION))));
  ...
  public void testUnthrottledWrite() throws Exception {
    ...
    MatcherAssert.assertThat(
        "Unthrottled write took " + (finish - start) + "ms, but expected to complete in " + UNTRHOTTLED_REQUEST_TIME_MS + "ms",
  ...
  public void testThrottledRead() throws Exception {
    ...
    long start = System.currentTimeMillis();
    final org.apache.http.HttpResponse response = httpClient.execute(new HttpHost("127.0.0.1", readWebServerPort), request);
    ...
    MatcherAssert.assertThat(
        "Expected throttled read to complete in approximately " + msToReadThrottled + "ms" + " but took " + (finish - start) + "ms",
        (double) (finish - start),
        both(greaterThan(msToReadThrottled * (1 - ALLOWABLE_VARIATION))).and(lessThan(msToReadThrottled * (1 + ALLOWABLE_VARIATION))));
  ...
  public void testUnthrottledRead() throws Exception {
    ...
    MatcherAssert.assertThat(
        "Unthrottled read took " + (finish - start) + "ms, but expected to complete in " + UNTRHOTTLED_REQUEST_TIME_MS + "ms",
        finish - start, lessThan((long) UNTRHOTTLED_REQUEST_TIME_MS));
  ...
  public void testChangeThrottling() throws IOException {
    ...
    org.apache.http.HttpResponse response = httpClient.execute(new HttpHost("127.0.0.1", readWebServerPort), request);
    byte[] readContent = new byte[100000];
    ...
    response = httpClient.execute(new HttpHost("127.0.0.1", readWebServerPort), request);
    ...
    MatcherAssert.assertThat(
        "Expected second read to take approximately half as long as first throttled read. First read took " + (firstFinish - firstStart)
            + "ms" + " but second read took " + (secondFinish - secondStart) + "ms",
  ...
  public void testDisableThrottling() throws IOException {
    ...
    MatcherAssert.assertThat(
        "Expected second read to complete within " + UNTRHOTTLED_REQUEST_TIME_MS + "ms, without throttling. First read took "
  ...
  private CloseableHttpClient createHttpClient(int proxyPort) {
