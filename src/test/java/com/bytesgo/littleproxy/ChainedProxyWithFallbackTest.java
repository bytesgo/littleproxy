...
/**
 * Tests a proxy chained to a missing downstream proxy. When the downstream proxy is unavailable, the downstream proxy
 * should just fall back to a direct connection.
 */
public class ChainedProxyWithFallbackTest extends BaseProxyTest {
  ...
  protected void setUp() {
    ...
    this.proxyServer = bootstrapProxy().withName("Downstream").withPort(0).withChainProxyManager(new ProxyChainManager() {
      @Override
