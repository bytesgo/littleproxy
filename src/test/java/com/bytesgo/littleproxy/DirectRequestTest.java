...
public class DirectRequestTest {
  ...
  public void testHttpsShouldCancelConnection() {
    ...
    try {
      ...
      MatcherAssert.assertThat("Expected an SSL exception when attempting to perform an HTTPS GET directly to the proxy", cause,
  ...
  public void testAllowRequestToOriginServerWithOverride() {
    // verify that the filter is hit twice: first, on the request from the client, without a Via header; and second, when
    // the proxy
    // forwards the request to itself
    final AtomicBoolean receivedRequestWithoutVia = new AtomicBoolean();

    proxyServer = DefaultHttpProxyServer.bootstrap().withPort(0).withAllowRequestToOriginServer(true)
        .withProxyAlias("testAllowRequestToOriginServerWithOverride").withFiltersSource(new HttpFilterSourceAdapter() {
          @Override
          public HttpFilter filterRequest(HttpRequest originalRequest) {
            return new HttpFilterAdapter(originalRequest) {
              @Override
