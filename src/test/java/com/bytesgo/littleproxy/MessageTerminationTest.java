...
public class MessageTerminationTest {
  ...
  public void testResponseWithoutTerminationIsChunked() throws Exception {
    ...
    mockServer.when(request().withMethod("GET").withPath("/"), Times.unlimited())
    ...
    MatcherAssert.assertThat("Expected to see a Transfer-Encoding header", transferEncodingHeaders.length, greaterThanOrEqualTo(1));
  ...
  public void testResponseWithContentLengthNotModified() throws Exception {
    ...
    Header[] transferEncodingHeaders = response.getHeaders("Transfer-Encoding");
    MatcherAssert.assertThat("Did not expect to see a Transfer-Encoding header", transferEncodingHeaders, emptyArray());
  ...
  public void testFilterAddsContentLength() throws Exception {
    ...
    proxyServer = DefaultHttpProxyServer.bootstrap().withFiltersSource(new HttpFilterSourceAdapter() {
      @Override
    ...
    MatcherAssert.assertThat("Expected to see a Content-Length header", contentLengthHeaders.length, greaterThanOrEqualTo(1));
  ...
  public void testResponseToHEADNotModified() throws Exception {
    ...
    mockServer.when(request().withMethod("HEAD").withPath("/"), Times.unlimited())
    ...
    Header[] contentLengthHeaders = response.getHeaders("Content-Length");
    MatcherAssert.assertThat("Did not expect to see a Content-Length header", contentLengthHeaders, emptyArray());
