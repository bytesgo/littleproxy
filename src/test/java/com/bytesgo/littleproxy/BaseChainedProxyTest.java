...
public abstract class BaseChainedProxyTest extends BaseProxyTest {
    ...
    protected final ActivityTracker DOWNSTREAM_TRACKER = new ActivityTrackerAdapter() {
        ...
        public void requestSentToServer(FullFlowContext flowContext,
            ...
            TRANSPORTS_USED.add(flowContext.getChainedProxy().getTransportProtocol());
        }
    };
    ...
    protected void setUp() {
        ...
        this.proxyServer = bootstrapProxy().withName("Downstream").withPort(0)
                .withChainProxyManager(proxyChainManager()).plusActivityTracker(DOWNSTREAM_TRACKER).start();
    }

    protected HttpProxyServerBootstrap upstreamProxy() {
        return DefaultHttpProxyServer.bootstrap().withName("Upstream").withPort(0)
    ...
    private void assertThatUpstreamProxyReceivedSentRequests() {
        assertEquals("Upstream proxy should have seen every request sent by downstream proxy",
                REQUESTS_SENT_BY_DOWNSTREAM.get(), REQUESTS_RECEIVED_BY_UPSTREAM.get());
        assertEquals("1 and only 1 transport protocol should have been used to upstream proxy", 1,
                TRANSPORTS_USED.size());
    ...
    protected class BaseChainedProxy extends ProxyChainAdapter {
        ...
        public InetSocketAddress getChainedProxyAddress() {
            try {
                return new InetSocketAddress(InetAddress.getByName("127.0.0.1"),
