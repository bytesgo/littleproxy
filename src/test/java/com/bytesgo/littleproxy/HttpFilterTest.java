...
public class HttpFilterTest {
    ...
    /**
     * Sets up the HttpProxyServer instance for a test. This method initializes the
     * proxyServer and proxyPort method variables, and should be called before
     * making any requests through the proxy server.
     *
     * The proxy cannot be created in an @Before method because the filtersSource
     * must be initialized by each test before the proxy is created.
     *
     * @param filtersSource HTTP filters source
     */
    private void setUpHttpProxyServer(HttpFilterSource filtersSource) {
    ...
    public void testFiltering() throws Exception {
        ...
        final AtomicLongArray serverToProxyResponseReceivingNanos = new AtomicLongArray(
                new long[] { -1, -1, -1, -1, -1 });
        final AtomicLongArray serverToProxyResponseReceivedNanos = new AtomicLongArray(
                new long[] { -1, -1, -1, -1, -1 });
        final AtomicLongArray proxyToServerConnectionQueuedNanos = new AtomicLongArray(
                new long[] { -1, -1, -1, -1, -1 });
        final AtomicLongArray proxyToServerResolutionStartedNanos = new AtomicLongArray(
                new long[] { -1, -1, -1, -1, -1 });
        final AtomicLongArray proxyToServerResolutionSucceededNanos = new AtomicLongArray(
                new long[] { -1, -1, -1, -1, -1 });
        final AtomicLongArray proxyToServerResolutionFailedNanos = new AtomicLongArray(
                new long[] { -1, -1, -1, -1, -1 });
        final AtomicLongArray proxyToServerConnectionStartedNanos = new AtomicLongArray(
                new long[] { -1, -1, -1, -1, -1 });
        final AtomicLongArray proxyToServerConnectionSSLHandshakeStartedNanos = new AtomicLongArray(
                new long[] { -1, -1, -1, -1, -1 });
        final AtomicLongArray proxyToServerConnectionFailedNanos = new AtomicLongArray(
                new long[] { -1, -1, -1, -1, -1 });
        final AtomicLongArray proxyToServerConnectionSucceededNanos = new AtomicLongArray(
                new long[] { -1, -1, -1, -1, -1 });
        final AtomicLongArray serverToProxyResponseTimedOutNanos = new AtomicLongArray(
                new long[] { -1, -1, -1, -1, -1 });
        ...
        final HttpFilterSource filtersSource = new HttpFilterSourceAdapter() {
                ...
                return new HttpFilterAdapter(originalRequest) {
                    ...
                    public HttpObject serverToProxyResponse(HttpObject httpObject) {
                        ...
                        if (httpObject instanceof HttpResponse) {
                            ((HttpResponse) httpObject).headers().set("Header-Pre", "1");
                        }
                        return httpObject;
                    }
                    ...
                    public HttpObject proxyToClientResponse(HttpObject httpObject) {
                        ...
                        if (httpObject instanceof HttpResponse) {
                            ((HttpResponse) httpObject).headers().set("Header-Post", "2");
                        }
                        return httpObject;
                    }
        ...
        Thread.sleep(500);
        assertEquals("Response should have included the custom header from our pre filter", "1",
                response1.getFirstHeader("Header-Pre").getValue());
        assertEquals("Response should have included the custom header from our post filter", "2",
                response1.getFirstHeader("Header-Post").getValue());
        ...
        MatcherAssert.assertThat(proxyToServerConnectionQueuedNanos.get(i),
                lessThan(proxyToServerResolutionStartedNanos.get(i)));
        MatcherAssert.assertThat(proxyToServerResolutionStartedNanos.get(i),
                lessThan(proxyToServerResolutionSucceededNanos.get(i)));
        MatcherAssert.assertThat(proxyToServerResolutionSucceededNanos.get(i),
                lessThan(proxyToServerConnectionStartedNanos.get(i)));
        ...
        MatcherAssert.assertThat(proxyToServerConnectionStartedNanos.get(i),
                lessThan(proxyToServerConnectionSucceededNanos.get(i)));
        MatcherAssert.assertThat(proxyToServerConnectionSucceededNanos.get(i),
                lessThan(proxyToServerRequestSendingNanos.get(i)));
        MatcherAssert.assertThat(proxyToServerRequestSendingNanos.get(i),
                lessThan(proxyToServerRequestSentNanos.get(i)));
        MatcherAssert.assertThat(proxyToServerRequestSentNanos.get(i),
                lessThan(serverToProxyResponseReceivingNanos.get(i)));
        MatcherAssert.assertThat(serverToProxyResponseReceivingNanos.get(i),
                lessThan(serverToProxyResponseReceivedNanos.get(i)));
        ...
        assertNotNull("Server channel context from proxyToServerConnectionSucceeded() should not be null",
                serverCtxReference.get());
        InetSocketAddress remoteAddress = (InetSocketAddress) serverCtxReference.get().channel().remoteAddress();
        assertNotNull("Server's remoteAddress from proxyToServerConnectionSucceeded() should not be null",
                remoteAddress);
        // make sure we're getting the right remote address (and therefore the right
        // server channel context) in the
    ...
    public void testResolutionStartedFilterReturnsUnresolvedAddress() throws Exception {
        ...
        HttpFilterSource filtersSource = new HttpFilterSourceAdapter() {
            @Override
            public HttpFilter filterRequest(HttpRequest originalRequest) {
                return new HttpFilterAdapter(originalRequest) {
                    @Override
                    ...
                    public void proxyToServerResolutionSucceeded(String serverHostAndPort,
                            InetSocketAddress resolvedRemoteAddress) {
                        assertFalse("expected to receive a resolved InetSocketAddress",
                                resolvedRemoteAddress.isUnresolved());
    ...
    public void testResolutionFailedCalledAfterDnsFailure() throws Exception {
        ...
        when(mockFailingResolver.resolve("www.doesnotexist", 80))
                .thenThrow(new UnknownHostException("www.doesnotexist"));

        this.proxyServer = DefaultHttpProxyServer.bootstrap().withPort(0).withFiltersSource(filtersSource)
                .withServerResolver(mockFailingResolver).start();

        HttpClientUtil.performHttpGet("http://www.doesnotexist/some-resource", proxyServer);
        Thread.sleep(500);

        // verify that the filters related to this functionality were correctly
        // invoked/not invoked as appropriate, but also
        // verify that
        // other filters were invoked/not invoked as expected
        assertFalse("proxyToServerResolutionSucceeded method was called but should not have been",
                filter.isProxyToServerResolutionSucceededInvoked());
        assertTrue("proxyToServerResolutionFailed method was not called",
                filter.isProxyToServerResolutionFailedInvoked());
        ...
        assertFalse("Expected filter method to not be called",
                filter.isProxyToServerConnectionSSLHandshakeStartedInvoked());
    ...
    public void testConnectionFailedCalledAfterConnectionFailure() throws Exception {
        ...
        // verify that
        // other filters were invoked/not invoked as expected
        assertFalse("proxyToServerConnectionSucceeded should not be called when connection fails",
                filter.isProxyToServerConnectionSucceededInvoked());
        assertTrue("proxyToServerConnectionFailed should be called when connection fails",
                filter.isProxyToServerConnectionFailedInvoked());

        assertTrue("Expected filter method to be called", filter.isClientToProxyRequestInvoked());
        // proxyToServerRequest is invoked before the connection is made, so it should
        // be hit
    ...
    /**
     * Verifies the proper filters are invoked when an attempt to connect to an
     * unencrypted upstream chained proxy fails.
     */
    @Test
    public void testFiltersAfterUnencryptedConnectionToUpstreamProxyFails() throws Exception {
        ...
        // the server doesn't have to exist, since the connection to the chained proxy
        // will fail
        ...
        // verify that
        // other filters were invoked/not invoked as expected
        assertFalse("proxyToServerConnectionSucceeded should not be called when connection to chained proxy fails",
                filter.isProxyToServerConnectionSucceededInvoked());
        assertTrue("proxyToServerConnectionFailed should be called when connection to chained proxy fails",
                filter.isProxyToServerConnectionFailedInvoked());
    ...
    /**
     * Verifies the proper filters are invoked when an attempt to connect to an
     * upstream chained proxy over SSL fails. (The proxyToServerConnectionFailed()
     * filter method is particularly important.)
     */
    @Test
    public void testFiltersAfterSSLConnectionToUpstreamProxyFails() throws Exception {
        // create an upstream chained proxy using the same SSL engine as the chained
        // proxy tests
        final HttpProxyServer chainedProxy = DefaultHttpProxyServer.bootstrap().withName("ChainedProxy").withPort(0)
                .withSslEngineSource(new SelfSignedSslEngineSource("chain_proxy_keystore_1.jks")).start();
    ...
    public void testResponseTimedOutInvokedAfterServerTimeout() throws Exception {
        mockServer.when(request().withMethod("GET").withPath("/servertimeout"), Times.once())
        ...
        this.proxyServer = DefaultHttpProxyServer.bootstrap().withPort(0).withFiltersSource(filtersSource)
                .withIdleConnectionTimeout(3).start();

        org.apache.http.HttpResponse httpResponse = HttpClientUtil
                .performHttpGet("http://localhost:" + mockServerPort + "/servertimeout", proxyServer);
        assertEquals("Expected to receive an HTTP 504 Gateway Timeout from proxy", 504,
    ...
    public void testRequestSentInvokedAfterLastHttpContentSent() throws Exception {
                ...
                return new HttpFilterAdapter(originalRequest) {
                    ...
                    public HttpResponse proxyToServerRequest(HttpObject httpObject) {
                        if (httpObject instanceof LastHttpContent) {
                            assertFalse(
                                    "requestSentCallback should not be invoked until the LastHttpContent is processed",
                                    requestSentCallbackInvoked.get());
                    ...
                    @Override
                    public void proxyToServerRequestSent() {
                        // proxyToServerRequestSent should only be invoked after the entire request,
                        // including payload, has been sent to the
                        // server
                        assertTrue(
                                "proxyToServerRequestSent callback invoked before LastHttpContent was received from the client and sent to the server",
                                lastHttpContentProcessed.get());
        ...
        // test with a POST request with a payload. post a large amount of data, to
        // force chunked content.
        HttpClientUtil.performHttpPost("http://localhost:" + webServerPort + "/", 50000, proxyServer);
        Thread.sleep(500);

        assertTrue("proxyToServerRequest callback was not invoked for LastHttpContent for chunked POST",
                lastHttpContentProcessed.get());
        assertTrue("proxyToServerRequestSent callback was not invoked for chunked POST",
                requestSentCallbackInvoked.get());
        ...
        assertTrue("proxyToServerRequest callback was not invoked for LastHttpContent for GET",
                lastHttpContentProcessed.get());
        assertTrue("proxyToServerRequestSent callback was not invoked for GET", requestSentCallbackInvoked.get());
    }

    /**
     * Verifies that the proxy properly handles a null HttpFilters instance, as
     * allowed in the
     * {@link HttpFilterSource#filterRequest(HttpRequest, ChannelHandlerContext)}
     * documentation.
     */
    @Test
    public void testNullHttpFilterSource() throws Exception {
        mockServer.when(request().withMethod("GET").withPath("/testNullHttpFilterSource"), Times.exactly(1))
        ...
        org.apache.http.HttpResponse httpResponse = HttpClientUtil
                .performHttpGet("http://localhost:" + mockServerPort + "/testNullHttpFilterSource", proxyServer);
        Thread.sleep(500);
    ...
    private long now() {
        // using nanoseconds instead of milliseconds, since it is extremely unlikely
        // that any two callbacks would be invoked in
        // the same nanosecond,
    ...
    /**
     * HttpFilters instance that monitors HttpFilters methods and tracks which
     * methods have been invoked.
     */
    private static class HttpFiltersMethodInvokedAdapter implements HttpFilter {
