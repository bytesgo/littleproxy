...
public class HttpFilterTest {
  ...
  public void testResolutionFailedCalledAfterDnsFailure() throws Exception {
    ...
    this.proxyServer =
        DefaultHttpProxyServer.bootstrap().withPort(0).withFiltersSource(filtersSource).withServerResolver(mockFailingResolver).start();

    HttpClientUtil.performHttpGet("http://www.doesnotexist/some-resource", proxyServer);
    Thread.sleep(500);

    // verify that the filters related to this functionality were correctly invoked/not invoked as appropriate, but also
    // verify that
  ...
  public void testResponseTimedOutInvokedAfterServerTimeout() throws Exception {
    ...
    this.proxyServer = DefaultHttpProxyServer.bootstrap().withPort(0).withFiltersSource(filtersSource).withIdleConnectionTimeout(3).start();
    ...
    HttpFilterSource filtersSource = new HttpFilterSourceAdapter() {
      ...
      public HttpFilter filterRequest(HttpRequest originalRequest) {
        return new HttpFilterAdapter(originalRequest) {
          ...
          @Override
          public void proxyToServerRequestSent() {
            // proxyToServerRequestSent should only be invoked after the entire request, including payload, has been sent to the
            // server
            assertTrue(
  ...
  private long now() {
    // using nanoseconds instead of milliseconds, since it is extremely unlikely that any two callbacks would be invoked in
    // the same nanosecond,
