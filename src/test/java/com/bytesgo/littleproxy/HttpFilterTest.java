...
public class HttpFilterTest {
  ...
  /**
   * Sets up the HttpProxyServer instance for a test. This method initializes the proxyServer and proxyPort method
   * variables, and should be called before making any requests through the proxy server.
   *
   * The proxy cannot be created in an @Before method because the filtersSource must be initialized by each test before
  ...
  public void testFiltering() throws Exception {
    ...
    MatcherAssert.assertThat(proxyToServerConnectionQueuedNanos.get(i), lessThan(proxyToServerResolutionStartedNanos.get(i)));
    MatcherAssert.assertThat(proxyToServerResolutionStartedNanos.get(i), lessThan(proxyToServerResolutionSucceededNanos.get(i)));
    MatcherAssert.assertThat(proxyToServerResolutionSucceededNanos.get(i), lessThan(proxyToServerConnectionStartedNanos.get(i)));
    ...
    MatcherAssert.assertThat(proxyToServerConnectionStartedNanos.get(i), lessThan(proxyToServerConnectionSucceededNanos.get(i)));
    MatcherAssert.assertThat(proxyToServerConnectionSucceededNanos.get(i), lessThan(proxyToServerRequestSendingNanos.get(i)));
    MatcherAssert.assertThat(proxyToServerRequestSendingNanos.get(i), lessThan(proxyToServerRequestSentNanos.get(i)));
    MatcherAssert.assertThat(proxyToServerRequestSentNanos.get(i), lessThan(serverToProxyResponseReceivingNanos.get(i)));
    MatcherAssert.assertThat(serverToProxyResponseReceivingNanos.get(i), lessThan(serverToProxyResponseReceivedNanos.get(i)));
  ...
  public void testResolutionFailedCalledAfterDnsFailure() throws Exception {
    ...
    this.proxyServer =
        DefaultHttpProxyServer.bootstrap().withPort(0).withFiltersSource(filtersSource).withServerResolver(mockFailingResolver).build();
    this.proxyServer.start();
  ...
  /**
   * Verifies the proper filters are invoked when an attempt to connect to an upstream chained proxy over SSL fails. (The
   * proxyToServerConnectionFailed() filter method is particularly important.)
   */
  @Test
  public void testFiltersAfterSSLConnectionToUpstreamProxyFails() throws Exception {
    ...
    final HttpProxyServer chainedProxy = DefaultHttpProxyServer.bootstrap().withName("ChainedProxy").withPort(0)
        .withSslEngineSource(new SelfSignedSslEngineSource("chain_proxy_keystore_1.jks")).build();
    chainedProxy.start();
  ...
  public void testResponseTimedOutInvokedAfterServerTimeout() throws Exception {
    ...
    this.proxyServer = DefaultHttpProxyServer.bootstrap().withPort(0).withFiltersSource(filtersSource).withIdleConnectionTimeout(3).build();
    this.proxyServer.start();

    org.apache.http.HttpResponse httpResponse =
        HttpClientUtil.performHttpGet("http://localhost:" + mockServerPort + "/servertimeout", proxyServer);
        ...
        return new HttpFilterAdapter(originalRequest) {
          ...
          public HttpResponse proxyToServerRequest(HttpObject httpObject) {
            if (httpObject instanceof LastHttpContent) {
              assertFalse("requestSentCallback should not be invoked until the LastHttpContent is processed",
   ...
   * {@link HttpFilterSource#filterRequest(HttpRequest, ChannelHandlerContext)} documentation.
   */
  @Test
  public void testNullHttpFilterSource() throws Exception {
    ...
    org.apache.http.HttpResponse httpResponse =
        HttpClientUtil.performHttpGet("http://localhost:" + mockServerPort + "/testNullHttpFilterSource", proxyServer);
    Thread.sleep(500);
