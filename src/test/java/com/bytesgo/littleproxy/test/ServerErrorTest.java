package com.bytesgo.littleproxy.test;
...
public class ServerErrorTest {
  ...
  public void testInvalidServerResponse() throws IOException {
    ...
    try (ServerSocket socket = createServerWithBadResponse()) {
      ...
      assertEquals("Expected to receive a 502 Bad Gateway after server responded with invalid response", 502,
  ...
  /**
   * Creates a ServerSocket that will read an HTTP request and response with an invalid response. NOTE: the ServerSocket
   * must be closed after the response is consumed.
   */
