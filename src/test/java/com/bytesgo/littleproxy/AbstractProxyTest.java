...
/**
 * Base for tests that test the proxy. This base class encapsulates all of the testing infrastructure.
 */
public abstract class AbstractProxyTest {
  ...
  /**
   * Override this to specify a username to use when authenticating with proxy.
   * 
   * @return
   */
  ...
  /**
   * Override this to specify a password to use when authenticating with proxy.
   * 
   * @return
   */
  ...
  protected ResponseInfo httpPostWithApacheClient(HttpHost host, String resourceUrl, boolean isProxied) throws Exception {
    ...
    final CloseableHttpClient httpClient = TestUtils.buildHttpClient();
    try {
      RequestConfig.Builder requestConfigBuilder = RequestConfig.custom();
      HttpContext httpContext = new BasicHttpContext();
      setProxyInfo(isProxied, requestConfigBuilder, httpContext, username, password);
      ...
      final HttpResponse response = httpClient.execute(host, request, httpContext);
  ...
  protected void setProxyInfo(boolean isProxied, RequestConfig.Builder requestConfigBuilder, HttpContext httpContext, String username,
      String password) {
    if (isProxied) {
  ...
  protected ResponseInfo httpGetWithApacheClient(HttpHost host, String resourceUrl, boolean isProxied, boolean callHeadFirst)
    ...
    String username = getUsername();
    String password = getPassword();
    CloseableHttpClient httpClient = TestUtils.buildHttpClient();
    try {
      RequestConfig.Builder requestConfigBuilder = RequestConfig.custom();
      HttpContext httpContext = new BasicHttpContext();
      setProxyInfo(isProxied, requestConfigBuilder, httpContext, username, password);
      ...
      if (callHeadFirst) {
        HttpRequestBase request = new HttpHead(resourceUrl);
        request.setConfig(requestConfigBuilder.build());
      ...
      if (contentLength != null) {
        assertEquals("Content-Length from GET should match that from HEAD", contentLength,
  ...
  protected String compareProxiedAndUnproxiedPOST(HttpHost host, String resourceUrl) throws Exception {
  ...
  protected String compareProxiedAndUnproxiedGET(HttpHost host, String resourceUrl) throws Exception {
    ResponseInfo proxiedResponse = httpGetWithApacheClient(host, resourceUrl, true, false);
    if (expectBadGatewayForEverything()) {
      assertReceivedBadGateway(proxiedResponse);
    } else {
  ...
  private void checkStatistics(HttpHost host) {
    ...
    MatcherAssert.assertThat(bytesReceivedFromClient.get(), greaterThan(0));
    assertEquals(numberOfExpectedClientInteractions, requestsReceivedFromClient.get());
    MatcherAssert.assertThat(bytesSentToServer.get(), greaterThan(0));
    assertEquals(numberOfExpectedServerInteractions, requestsSentToServer.get());
    MatcherAssert.assertThat(bytesReceivedFromServer.get(), greaterThan(0));
    assertEquals(numberOfExpectedServerInteractions, responsesReceivedFromServer.get());
    MatcherAssert.assertThat(bytesSentToClient.get(), greaterThan(0));
    assertEquals(numberOfExpectedClientInteractions, responsesSentToClient.get());
  }
  ...
  protected HttpProxyServerBootstrap bootstrapProxy() {
    return DefaultHttpProxyServer.bootstrap().plusActivityTracker(new ActivityTracker() {
      @Override
      ...
      @Override
      public void responseReceivedFromServer(FullFlowContext flowContext, io.netty.handler.codec.http.HttpResponse httpResponse) {
      ...
      @Override
      public void responseSentToClient(FlowContext flowContext, io.netty.handler.codec.http.HttpResponse httpResponse) {
