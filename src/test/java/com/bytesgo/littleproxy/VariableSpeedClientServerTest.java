 ...
 * 
 * Ignored because this doesn't quite trigger OOME for some reason. It also takes too long to include in normal tests.
 */
@Ignore
public class VariableSpeedClientServerTest {
  ...
  private void doTest(int port, int proxyPort, boolean slowServer) throws Exception {
    ...
    Thread.yield();
    HttpProxyServer proxyServer = DefaultHttpProxyServer.bootstrap().withPort(proxyPort).build();
    proxyServer.start();
    Thread.yield();
    Thread.sleep(400);
  ...
  private void startServer(final int port, final boolean slowReader) throws Exception {
  ...
  private void startServerOnThread(int port, boolean slowReader) throws Exception {
