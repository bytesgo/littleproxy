...
public class TestUtils {
  ...
  /**
   * Creates and starts an embedded web server on a JVM-assigned HTTP ports. Each response has a body that indicates how
   * many bytes were received with a message like "Received x bytes\n".
   * 
  ...
  /**
   * Creates and starts an embedded web server on a JVM-assigned HTTP ports. Creates and starts embedded web server that
   * is running on given port. Each response has a body that contains the specified contents.
   *
   * @return Instance of Server
   */
  ...
  public static Server startWebServer(boolean enableHttps) {
    ...
    if (enableHttps) {
      ...
      connector.setPort(0);
      /*
       * <p>Ox: For some reason, on OS X, a non-zero timeout can causes sporadic issues. <a
       * href="http://stackoverflow.com/questions /16191236/tomcat-startup-fails
       * -due-to-java-net-socketexception-invalid-argument-on-mac-o">This StackOverflow thread</a> has some insights into it,
       * but I don't quite get it.</p>
       * 
       * <p>This can cause problems with Jetty's SSL handshaking, so I have to set the handshake timeout and the maxIdleTime
       * to 0 so that the SSLSocket has an infinite timeout.</p>
       */
  ...
  static HttpClientConnectionManager createConnectionManager() {
    ...
    PoolingHttpClientConnectionManager cm =
        new PoolingHttpClientConnectionManager(sockRegistryBuilder.build(), null, null, null, 60, TimeUnit.SECONDS);
    return cm;
  }
