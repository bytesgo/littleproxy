...
public class ServerGroupTest {
  ...
  public void testSingleWorkerThreadPoolConfiguration() throws ExecutionException, InterruptedException {
    ...
    mockServer.when(request().withMethod("GET").withPath(firstRequestPath), Times.exactly(1))
        .respond(response().withStatusCode(200).withBody("first").withDelay(TimeUnit.MILLISECONDS, 500));

    mockServer.when(request().withMethod("GET").withPath(secondRequestPath), Times.exactly(1))
        .respond(response().withStatusCode(200).withBody("second"));
    ...
    assertEquals("Expected clientToProxy filter methods to be executed on the same thread for both requests", firstClientThreadName.get(),
        secondClientThreadName.get());
    assertEquals("Expected serverToProxy filter methods to be executed on the same thread for both requests", firstProxyThreadName.get(),
        secondProxyThreadName.get());
  }
